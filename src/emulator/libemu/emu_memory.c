...
#define EMU_PAGE_SIZE (1 << PAGE_BITS)
...
void emu_memory_segment_set(struct emu_memory *em, int segment, unsigned long value)
{
    em->segment_table[segment] = value;
}
...
static inline int page_alloc(struct emu_memory *em, uint32_t addr)
{
	...
	if( em->pagetable[PAGESET(addr)][PAGE(addr)] == NULL )
	{
		em->pagetable[PAGESET(addr)][PAGE(addr)] = malloc(EMU_PAGE_SIZE);
		...
		memset(em->pagetable[PAGESET(addr)][PAGE(addr)], 0, EMU_PAGE_SIZE);
	}
...
// ---------------------------- EXECUTION ----------------------------
int32_t emu_memory_execute_byte(struct emu_memory *m, uint32_t addr, uint8_t *byte)
{
    memory_access_struct m_str;
    addr += m->segment_offset;
    translate_address(&m_str, addr, VM_EXEC);
    
    if( m_str.address == NULL )
	{
		if (LOGLVL >= DBGLOG) emu_errno_set(m->emu, EFAULT);
		if (LOGLVL >= DBGLOG) emu_strerror_set(m->emu, "error accessing 0x%08x not mapped\n", addr);
		return -1;
	}
    
    *byte = *((uint8_t *)m_str.address);
	
	return 0;
}

int32_t emu_memory_execute_word(struct emu_memory *m, uint32_t addr, uint16_t *word)
{
#if BYTE_ORDER == BIG_ENDIAN
	uint16_t val;
	int32_t retval = emu_memory_execute_block(m, addr, &val, 2);
	val =  ((val & 0xff00) >> 8) | 
		   ((val & 0x00ff) << 8);
	bcopy(&val,word,2);
	return retval;
#else
	return emu_memory_execute_block(m, addr, word, 2);
#endif
}

int32_t emu_memory_execute_dword(struct emu_memory *m, uint32_t addr, uint32_t *dword)
{
#if BYTE_ORDER == BIG_ENDIAN
	uint32_t val;
	int32_t retval =  emu_memory_execute_block(m, addr, &val, 4);
	...
	return retval;
#else
	return emu_memory_execute_block(m, addr, dword, 4);
#endif
}

int32_t emu_memory_execute_block(struct emu_memory *m, uint32_t addr, void *dest, size_t len)
{
    memory_access_struct m_str;
    addr += m->segment_offset;
    translate_address(&m_str, addr, VM_EXEC);
	...
	if (m_str.address + len < m_str.last_address_in_vma)
	{
		memcpy(dest, m_str.address, len);
		return 0;
	}
	else
	{
		uint32_t cb = m_str.last_address_in_vma - m_str.address;
		memcpy(dest, m_str.address, cb);
		return emu_memory_execute_block(m, (uint32_t)(m_str.address + cb), dest + cb, len - cb);
	}
}
// ---------------------------- EXECUTION ----------------------------
...
int32_t emu_memory_read_byte(struct emu_memory *m, uint32_t addr, uint8_t *byte)
{
    memory_access_struct m_str;
    addr += m->segment_offset;
    translate_address(&m_str, addr, VM_READ);
...
int32_t emu_memory_read_block(struct emu_memory *m, uint32_t addr, void *dest, size_t len)
{
	...
	else
	{
		uint32_t cb = m_str.last_address_in_vma - m_str.address;
		memcpy(dest, m_str.address, cb);
		return emu_memory_read_block(m, (uint32_t)(m_str.address + cb), dest + cb, len - cb);
	}
}
...
int32_t emu_memory_write_byte(struct emu_memory *m, uint32_t addr, uint8_t byte)
{
    memory_access_struct m_str;
    addr += m->segment_offset;
    translate_address(&m_str, addr, VM_WRITE);
    ...
    //*((uint8_t *)m_str.address) = byte;
    safe_memcpy( (void*)m_str.address, (void*)(&byte), 1);
...
int32_t emu_memory_write_block(struct emu_memory *m, uint32_t addr, const void *src, size_t len)
{
	...
	if (m_str.address + len < m_str.last_address_in_vma)
	{
        safe_memcpy(m_str.address, src, len);
		return 0;
	}
	else
	{
		uint32_t cb = m_str.last_address_in_vma - m_str.address;
        safe_memcpy(m_str.address, src, cb);
		return emu_memory_write_block(m, (uint32_t)(m_str.address + cb), src + cb, len - cb);
	}
