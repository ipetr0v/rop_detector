...
//	logDebug(c->emu,"decoding\n");
    if (LOGLVL >= DBGLOG) emu_cpu_debug_print(c); // --- DEBUG OUTPUT ---

	uint8_t dis[32];
	//emu_memory_execute_block(c->mem,c->eip,dis,32);
    if ( emu_memory_execute_block(c->mem,c->eip,dis,32) )
    {
        return -1;
    }
	...
	//{
	if (LOGLVL >= DBGLOG) expected_instr_size = dasm_print_instruction(c->eip,dis,0,c->instr_string); // --- DEBUG OUTPUT ---
	//}
    
    if (LOGLVL >= DBGLOG) printf("Ins: %s \n", c->instr_string); // --- DEBUG OUTPUT ---
	...
	while( 1 )
	{
		...
		else
		{
			...
			if( c->cpu_instr_info->format.fpu_info == 0 )
			{
                ...
                // --- Not supported instructions become NOP ---
...
int32_t emu_cpu_step(struct emu_cpu *c)
{
	...
	/* call the function */
    // --- Not supported instructions become NOP ---
    if ( c->cpu_instr_info->function == NULL )
        return 0;
